...
| Add or retune an OWNABLE colour set, or change what unlocks one | [content_themes.h](content_themes.h) — the CFG picker's rows, their display names and the one line each says it is come by. The hexes live in `assets/PAL_CORE.json`; this table is only which sets are OFFERED and how. Ownership is NOT stored: a `Chip` row unlocks off the ever-held item shelf, so the chip can be spent and the set stays. The accessible sets are `Start` and never behind a draw. `test_theme_table_covers_the_palette` is the gate — every authored set has exactly one row here. |
...
| Change what a guardian IS, what it SAYS, or what beating one teaches | [areas/README.md](areas/README.md) — a guardian is one `GuardianDef` row on its area: its banner, its own move, and its `GuardianLine` pairs (what it says in the Cant, and what the pet can see it doing — the second is always plain, because you can read a body without sharing a language). How far it out-classes the area is the same step everywhere (`kGuardian*`, `tunables.h`), per rule 2 below. |
