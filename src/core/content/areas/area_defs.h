...
// itself: its name/tier/Title, its 5 sub-area names, its 5 sub-area boss names +
...
struct AreaDef {
    ...
    int tier;          // difficulty tier — drives boss scaling + the Bits payout rank
...
// define its AreaDef here-ish) and add one entry below — kAreaCount follows
// automatically.
inline constexpr const AreaDef* const kAreaList[] = {
    &kAreaCitrusCircuit, &kAreaPirateBayou, &kAreaNapstorrentMoors,
    &kAreaCastleRapidscare,
};
