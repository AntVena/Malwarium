...
| Change an area's GUARDIAN — who watches it, and what beating one teaches | its `area.cpp`'s `guardian` row (`GuardianDef`). A guardian is met on the WALK, not the ladder, so it has no sub-area and no clear flag; see *The guardian* in [AREA_CONTENT_STANDARD.md](AREA_CONTENT_STANDARD.md) |
