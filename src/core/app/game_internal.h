...
// The owned-recipe bitset's capacity is kMergeRecipeWireCap, declared in
// game_rig_shop.h so that game.h can size Game::recipesOwned_ without seeing this
// private header. The first 32 wires ALSO ride the legacy u32
// (SaveData::recipesUnlocked), so a build that predates the bitset still reads the
// recipes it knows about.
