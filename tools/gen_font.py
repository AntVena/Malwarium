...
It owns the TABLE only. The renderer over it (drawText, the word-wrap rule) is
hand-written in font.cpp and knows nothing about which face it is drawing.
...
def emit(glyphs):
    lines = [
        ...
        "",
        f"const uint8_t kGlyphs[{len(glyphs)}][{CELL}] = {{",
    ]
...
def main():
    ...
    with open(OUT, "w") as f:
        f.write(emit(glyphs))
    print(f"wrote {OUT} — {len(glyphs)} glyphs, {len(glyphs) * CELL} bytes")
