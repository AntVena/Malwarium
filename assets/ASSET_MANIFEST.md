...
- **Snapping a Metamorphic sheet takes NEITHER `--accent` NOR the eye accent in its palette.** Both
  ...
  from the idle's own two, anchored on the accent CLUSTERS the trip returns rather than on a fixed
  box: the drifted accent survives the trip as one distinct colour and is easy to find before the
  snap eats it, and anchoring per frame is what lets a row whose head moves — a strike, a flinch —
  take the same stamp as a row whose head does not. Discard clusters below a couple of pixels; the
  drift leaves single specks elsewhere on the body that are not eyes.
- **STAMP THE FRAGILE FEATURE BACK; DO NOT PROMPT FOR IT.** The restamp above generalises, and it
  ...
  does not fit the cell and has to be dropped rather than scaled. Applied to that sheet's walk row
  it takes all eight returned frames from 58-72 rows tall back under the cell's 64.
- **A short prompt plus a negative one is what steers a MOTION; a long prompt steers a drawing.**
  `animate_game_art` reads the source for the creature, so the useful prompt for a clip row is the
  clip's own name — `Attack`, `Hurt`, `Walk` — with the failure named in `negative_prompt` rather
  than the pose spelled out in the positive one. Naming the gait's alternative there (`swimming,
  floating, hovering, drifting, water`) is what keeps an eight-armed walker striding instead of
  swimming. The frames come back at the source's own pixel scale, so they crop into the cell with
  no resampling — but they come back FRAMED wider and taller than they went in, and a row is
  usable only as far as the cell can hold what the trip added.
- **Not every returned frame is a clip frame.** An eight-frame return is a loop through one motion,
  so its ends sit near the rest pose and only its middle is the thing asked for: a strike row is
  the columns where the arms are already out, and the two or three that read as gathering are
  wind-up, which `frameAt` cannot show in order (`src/core/content/defs.h`). Pick for the phase,
  then order the picks so the row still closes into a cycle. Anything the pick leaves behind is a
  frame the row is better without, not a column to be filled later.
- **Fragments are a fault by definition on a single-mass creature.** The redraw resolves
  overlapping limbs afresh in every frame, and where it loses a join it leaves a tip behind as a
  floating chip of a few pixels. Nothing on these sheets is detached, so dropping every connected
  component but the largest, per cell, is safe and mechanical.
...
| `ICON_SECTOR_<AREA_ID>` | Per-area row glyph, EXPL zone picker | 20×20 | one per area, named on that area's own `AreaDef::icon`; the two ENDLESS zones have no AreaDef and name `ICON_SECTOR_DEEPWEB_DIVE` / `ICON_SECTOR_DARKWEB_CRAWL` from their own `areas/*/area.cpp` instead | ☑ | `/assets/icons/ICON_SECTOR_*.png` |
| `ICON_SECTOR_SILK_LODE` | THE SILK LODE's row glyph | 20×20 | the sixth area — a seam of silk in rock, read as a vein rather than as a spider (the spider is the guardian's, and a flock is how a guardian draws). Named on its row; until it exists the EXPL row draws `drawIconSlot`'s empty frame, which means "art pending" and not "no zone" | ☑ | `/assets/icons/ICON_SECTOR_SILK_LODE.png` |
| `ICON_SECTOR_DARKWEB_CRAWL` | THE DARKWEB CRAWL's row glyph | 20×20 | the terminal endless zone, under the Dive's row. Wants to read as the Dive's opposite number at a glance — the Dive descends, this one spreads | ☑ | `/assets/icons/ICON_SECTOR_DARKWEB_CRAWL.png` |
...
| `ICON_SYS_BATTERY` | Battery status glyph | 16×16 | the Hacker face's battery readout, beside the percentage. A **5-frame strip**, frame index == fill level (empty shell .. four bars) rather than an animation — the engine picks the frame from the smoothed reading (`core/app/power_status.h`), and the bar count is the non-colour channel that keeps the readout legible in grayscale. System Info states the same battery in words on its `BATT` line and draws no glyph | ☑ | `/assets/icons/ICON_SYS_BATTERY.png` |
...
| **Achievement icons** | Per-achievement glyphs, named on each row's `icon` field (`src/core/content/content_achievements.cpp`) | **GENERATED — `tools/gen_ach_icons.py` owns these PNGs; never hand-edit one, the next run reverts it. The `ach_icons` ctest is the drift guard.** The tool holds the grammar below as code: motifs are ASCII in its source, footers (`tally(n)` / `bar()` / `chevron()` / `shelf()`) are measured off the shipped family, and a rung added to an existing ladder `inherit()`s that ladder's motif rather than redrawing it. **One bespoke `ICON_ACH_*` glyph per row, no reuse.** Most are LADDERS, so they are one motif plus a centred tally — five skulls for the boss tiers, five depth arrows, four wifi arcs, three narrowing DEFRAG stacks, three arcade joysticks — countable before anything else, which is what lets a grid of them read in grayscale. The four creature lines are sets rather than ladders, so they use the line's own mark (lock / hook / horse / rearing worm) under a footer: a bar for "raised them all", a chevron for "took one deep", and — for the ARCH rows that ask for the same set alive on the shelf at the same time — a `shelf()`, which is that same bar bracketed by two uprights. That fourth footer exists because the distinction is invisible otherwise: `FULL_LINE_WORM` ("raised every one of them") and `WING_WORM` ("all five of them frozen at once") wear the same mark and would otherwise be the same picture. `DEVTOOLS_INTRUDER` is deliberately a question mark over a redaction bar — the 'Pedia draws a row's icon even while the row is masked, so anything representational there would spoil the one achievement whose point is working it out. The three Backup Drive rows are the other exception: a SET, not a ladder, so all three carry the same platter and differ only in its fate — restored and carried up, restored and going back down, or split with the pieces scattered. The Metamorphic pair are a set of the same kind, both the line's bell: solid and parted for the cabinet it is played on (§Q), hollow for the hatch run that was never spotted, and behind full-width bars for the deep cabinet score | ☑ | `/assets/icons/ICON_ACH_*.png` |
